...
export const PageDashboard: React.FC<PageDashboardProps> = ({
  ...
  const impressionsData = getInsightData("page_impressions_unique", "day");
  // Page reach
  const reachData = getInsightData("page_reach", "week");
  // Engaged users
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="text-sm text-blue-700">
                ...
                {insights ? "Available" : "Not available"}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {impressionsData.current.toLocaleString()}
                </p>
              </div>
            ...
            {impressionsData.change !== "N/A" && (
              <div
                className={`flex items-center mt-2 text-sm ${
                  impressionsData.change.startsWith("+")
                    ? "text-green-600"
                    : impressionsData.change.startsWith("-")
                    ? "text-red-600"
                    : "text-gray-600"
              ...
              >
                ...
                <span>
                  {impressionsData.change} {getComparisonText("day")}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {reachData.current.toLocaleString()}
                </p>
              </div>
            ...
            {reachData.change !== "N/A" && (
              <div
                className={`flex items-center mt-2 text-sm ${
                  reachData.change.startsWith("+")
                    ? "text-green-600"
                    : reachData.change.startsWith("-")
                    ? "text-red-600"
                    : "text-gray-600"
              ...
              >
                <TrendingUp className="w-4 h-4 mr-1" />
                <span>
                  {reachData.change} {getComparisonText("week")}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {engagedUsersData.current.toLocaleString()}
                </p>
            ...
            {engagedUsersData.change !== "N/A" && (
              <div
                className={`flex items-center mt-2 text-sm ${
                  engagedUsersData.change.startsWith("+")
                    ? "text-green-600"
                    : engagedUsersData.change.startsWith("-")
                    ? "text-red-600"
                    : "text-gray-600"
              ...
              >
                <TrendingUp className="w-4 h-4 mr-1" />
                <span>
                  {engagedUsersData.change} {getComparisonText("day")}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">Paid Impressions</p>
                <p className="text-2xl font-bold text-gray-900">
                  {paidImpressionsData.current.toLocaleString()}
                </p>
              </div>
            ...
            {paidImpressionsData.change !== "N/A" && (
              <div
                className={`flex items-center mt-2 text-sm ${
                  paidImpressionsData.change.startsWith("+")
                    ? "text-green-600"
                    : paidImpressionsData.change.startsWith("-")
                    ? "text-red-600"
                    : "text-gray-600"
